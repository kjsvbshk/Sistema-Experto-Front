...
interface EditFailureModalProps {
    ...
    isOpen: boolean;
    onClose: () => void;
    onFailureUpdated: () => void;
}

interface EditFailureFormData {
    name: string;
    description: string;
    [key: string]: string;
}
...
export default function EditFailureModal({ failure, isOpen, onClose, onFailureUpdated }: EditFailureModalProps) {
    ...
    const { formState, isSubmitting, submitError, setValue, setTouched, handleSubmit, resetForm } = useForm({
        ...
        onSubmit: async (values) => {
            try {
                await failureService.updateFailure(failure.id, {
                    name: values.name,
                    description: values.description || undefined,
                });

                showSuccess('Falla actualizada exitosamente');
                resetForm();
                onFailureUpdated();
                onClose();
            } catch (error) {
                console.error('Error al actualizar falla:', error);
                showError('Error al actualizar falla');
            }
        },
    });
    ...
    // Actualizar valores del formulario cuando cambie la falla
    useEffect(() => {
        if (isOpen && failure) {
            setValue('name', failure.name);
            setValue('description', failure.description || '');
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [isOpen, failure]);
            ...
            <div className="relative w-full max-w-md shadow-2xl rounded-lg bg-gray-800 border border-gray-700 transform transition-all duration-200 ease-out">
                <div className="p-6">
                    ...
                    <div className="flex items-center justify-between mb-6">
                        <h3 className="text-lg font-semibold text-white">Editar Falla</h3>
                        <button
                            onClick={onClose}
