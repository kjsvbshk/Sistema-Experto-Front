...
interface CreateFactFormData {
    code: string;
    description: string;
    [key: string]: string;
}

export default function CreateFactModal({ isOpen, onClose, onFactCreated }: CreateFactModalProps) {
    console.log('🔍 CreateFactModal: Component rendered with isOpen:', isOpen);
    ...
    const { formState, isSubmitting, submitError, setValue, setTouched, handleSubmit, resetForm, getValues } = useForm({
        ...
        onSubmit: async (values) => {
            try {
                await factService.createFact({
                    code: values.code,
                    description: values.description,
                });

                showSuccess('Hecho creado exitosamente');
                resetForm(); // Limpiar el formulario
                onFactCreated();
                onClose();
            } catch (error) {
                console.error('Error al crear hecho:', error);
                showError('Error al crear hecho');
            }
        },
    });
    ...
    console.log('🔍 CreateFactModal: formValues:', formValues);
    console.log('🔍 CreateFactModal: formState:', formState);

    if (!isOpen) {
        console.log('🔍 CreateFactModal: Modal not open, returning null');
        return null;
    }

    console.log('🔍 CreateFactModal: Modal is open, rendering modal');
            ...
            <div className="relative w-full max-w-md shadow-2xl rounded-lg bg-gray-800 border border-gray-700 transform transition-all duration-200 ease-out">
                <div className="p-6">
                    ...
                    <div className="flex items-center justify-between mb-6">
                        <h3 className="text-lg font-semibold text-white">Crear Nuevo Hecho</h3>
                        <button
                            onClick={onClose}
